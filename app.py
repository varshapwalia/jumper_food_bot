...
from controllers.clover import clover

app = Flask(__name__)
CORS(app)

# Check Python Server
PYTHON_SERVER = os.environ.get('PYTHON_SERVER', 'DEVELOPMENT')
...
#     def __init__(self, userid=None, full_name=None, username=None, password=None, roles=None, role_name=None, block=False):
#         self.userid = userid
...
#         return SessionUser(userid=str(user.id), full_name=user.full_name, username=user.email,password=user.password, roles=user.userrole.allowed, role_name=user.userrole.name, block = user.block)
#     return None
...
#             if user and (not user.is_block()) and user.check_password(password):
...
#                 return redirect('/admin_welcome')
#         return render_template('admin/login.html', login_error="Username or passowrd not found")
#     return render_template('admin/login.html', login_error="")
